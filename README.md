...
**Unlock 1.1.4** — Windows-приложение, которое объединяет DPI bypass, локальный Telegram-прокси и VPN-подключения в одном интерфейсе. Оно рассчитано на обычную установку: скачайте `UnlockSetup.exe`, выберите нужные ярлыки и дождитесь завершения.
...
Переключатель **AI-сервисы** повторяет механизм Zapret-GUI 2.1.1: добавляет домены AI-провайдеров в DPI-обход, загружает полный `hosts`-пакет `dns.malw.link` и дополнение Goida-AI-Unlocker, проверяет его и кэширует в `%APPDATA%\Unlock\ai-hosts.txt`. Как и в исходной реализации, пакет содержит не только AI-сопоставления, но и дополнительные переопределения сайтов и блокировку рекламы через `0.0.0.0`. Из пакета удаляются адреса GitHub, необходимые самому обновлению. Отключение удаляет только блок Unlock и не затрагивает пользовательские или чужие строки `hosts`.
...
```powershell
py -B tools/check_release_version.py --tag v1.1.4
```
...
py -B tools/build_unlock_setup.py `
  --zip dist/Unlock.zip `
  --version 1.1.4 `
  --package-url https://github.com/voblya-dev/unlock/releases/download/v1.1.4/Unlock.zip `
