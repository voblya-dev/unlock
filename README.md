...
Переключатель **AI-сервисы** добавляет домены AI-провайдеров в DPI-обход и загружает актуальные IP-сопоставления из источников Zapret-GUI. Перед применением список строго фильтруется: в Windows `hosts` попадают только публичные IP и домены разрешённых AI-сервисов — OpenAI, Claude, Gemini, Copilot, Perplexity, Grok и другие. Сопоставления кэшируются в `%APPDATA%\Unlock\ai-hosts.txt`, поэтому режим можно повторно применить при временной недоступности источника. Отключение удаляет только записи, которыми управляет Unlock, и не затрагивает добавленные пользователем правила или другие строки `hosts`.
...
```powershell
py -B tools/check_release_version.py --tag v1.1.3
```
...
py -B tools/build_unlock_setup.py `
  --zip dist/Unlock.zip `
  --version 1.1.3 `
  --package-url https://github.com/voblya-dev/unlock/releases/download/v1.1.3/Unlock.zip `
